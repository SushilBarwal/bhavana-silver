...
const WhyChooseUs = ({ items: apiFeatures, loading }) => {
  ...
  const staticFeatures = [
    ...
    },
    {
      ...
      description:
        "Professional visuals provided for easy listing on your store",
    },
  ];

  // Use API features or fallback to static data
  // API structure: { id, icon (SVG URL), main_title, heading, description }
  // Component needs: { id, icon (component or URL), title, description }
  const features =
    apiFeatures && apiFeatures.length > 0
      ? apiFeatures.map((f) => ({
          id: f.id,
          iconUrl: f.icon, // SVG URL from API
          title: f.heading,
          description: f.description,
          mainTitle: f.main_title, // Extract main title if present
        }))
      : staticFeatures;

  // Extract section title from first feature with main_title
  const sectionTitle =
    apiFeatures && apiFeatures.length > 0
      ? apiFeatures.find((f) => f.main_title)?.main_title || "WHY CHOOSE US?"
      : "WHY CHOOSE US?";
